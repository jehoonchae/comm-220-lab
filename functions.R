run_model = function(agent_states,
                     edge_list,
  ...
  for (i in 1:rounds_of_interaction) {
    ...
    if (nrow(edge_list) > 0) {
      # in case there are no edges at all!
...
make_connections = function(n_agents,
                            n_delete = 0,
                            type = 'random',
                            del_self = TRUE) {
  if (type == 'random') {
    connection_matrix = matrix(runif(n_agentsnts * n_agents), nrow = n_agents)
  } else if (type == 'ones') {
